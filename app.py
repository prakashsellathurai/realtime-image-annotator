...
""")
    ...
    if not st.checkbox(
            "MAYBE DOWNLOAD 250MB OF DATA TO THE SERVER. THIS MIGHT TAKE A FEW MINUTES!"
    ):
        return
    ...
    if os.path.exists(file_path):
        if "size" not in EXTERNAL_DEPENDENCIES[file_path]:
            return
                ...
                file_path) == EXTERNAL_DEPENDENCIES[file_path]["size"]:
            return
    ...
    try:
        ...
        with open(file_path, "wb") as output_file:
            with urllib.request.urlopen(
                    EXTERNAL_DEPENDENCIES[file_path]["url"]) as response:
...
def run_the_app():
    ...
    def create_summary(metadata):
        one_hot_encoded = pd.get_dummies(metadata[["frame", "label"]],
                                         columns=["label"])
        summary = one_hot_encoded.groupby(["frame"]).sum().rename(
            columns={
                "label_biker": "biker",
                "label_car": "car",
    ...
    draw_image_with_boxes(
        image, yolo_boxes, "Real-time Computer Vision",
...
def get_selected_frames(summary, label, min_elts, max_elts):
    return summary[np.logical_and(summary[label] >= min_elts,
                                  summary[label] <= max_elts)].index
...
def object_detector_ui():
    ...
    confidence_threshold = st.sidebar.slider("Confidence threshold", 0.0, 1.0,
                                             0.5, 0.01)
    overlap_threshold = st.sidebar.slider("Overlap threshold", 0.0, 1.0, 0.3,
                                          0.01)
...
def draw_image_with_boxes(image, boxes, header, description):
    ...
    for _, (xmin, ymin, xmax, ymax, label) in boxes.iterrows():
        image_with_boxes[int(ymin):int(ymax),
                         int(xmin):int(xmax), :] += LABEL_COLORS[label]
